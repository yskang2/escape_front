...
const Anpractice = () => {
  ...
  useEffect(() => {
    // API에서 게시판 목록을 가져옵니다.
    axios.get('/board/anpracticeposts')
      .then((response) => {
  ...
  const handleRowSelected = (args) => {
    ...
    navigate(`/anpractice/post/${postId}`);
  };

  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
      <Header
        category={currentLanguage === 'Eng' ? 'Board (PERSONAL)' : '게시판(개인)'}
        title={currentLanguage === 'Eng' ? 'Group2' : 'Group2'}
      />
      <GridComponent
        dataSource={data}
        width="auto"
      ...
      <div className="mt-4 flex justify-end">
        <button type="button" onClick={() => navigate('/anpractice/create')} className="bg-blue-500 text-white px-6 py-2 rounded-md">{currentLanguage === 'Eng' ? 'Create Post' : '글쓰기'}</button>
      </div>
    </div>
  );
};

export default Anpractice;
