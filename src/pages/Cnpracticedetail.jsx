...
const Cnpracticedetail = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`http://54.180.179.9:5002/board/cnpracticeposts/${id}`);
        setPost(response.data);

        const commentsResponse = await axios.get(`http://54.180.179.9:5002/board/posts/${id}/comments`);
        setComments(commentsResponse.data);

        await axios.put(`http://54.180.179.9:5002/board/cnpracticeposts/${id}/increase-view`);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6">{currentLanguage === 'Eng' ? 'Group1 Board' : 'Group1'}</h1>
...
export default Cnpracticedetail;
