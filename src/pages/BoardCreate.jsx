...
const BoardCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('/board/posts', newPost);
      navigate('/board');
    } catch (error) {
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
      <h1 className="text-3xl font-bold mb-6">{currentLanguage === 'Eng' ? 'Free Board write' : '자유 게시판 글쓰기'}</h1>
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end">
          <button type="button" onClick={() => navigate('/board')} className="bg-gray-500 text-white px-6 py-2 rounded-md mr-2">{currentLanguage === 'Eng' ? 'cancel' : '취소'}</button>
