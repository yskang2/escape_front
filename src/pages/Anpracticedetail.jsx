...
const Anpracticedetail = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`http://54.180.179.9:5002/board/anpracticeposts/${id}`);
        setPost(response.data);

        const commentsResponse = await axios.get(`http://54.180.179.9:5002/board/posts/${id}/comments`);
        setComments(commentsResponse.data);

        await axios.put(`http://54.180.179.9:5002/board/anpracticeposts/${id}/increase-view`);
      } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6">
      <h1 className="text-3xl font-bold mb-6">{currentLanguage === 'Eng' ? 'Group2 Board' : 'Group2'}</h1>
      ...
      <div className="mt-4 flex justify-end">
        <button type="button" onClick={() => navigate('/anpractice')} className="bg-gray-500 text-white px-6 py-2 rounded-md mr-2">{currentLanguage === 'Eng' ? 'back' : '이전'}</button>
      </div>
    </div>
  );
};

export default Anpracticedetail;
