...
const Cnpractice = () => {
  ...
  useEffect(() => {
    // API에서 게시판 목록을 가져옵니다.
    axios.get('/board/cnpracticeposts')
      .then((response) => {
        console.log('123');
  ...
  const handleRowSelected = (args) => {
    ...
    navigate(`/cnpractice/post/${postId}`);
  };
  ...
  return (
    <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
      <Header
        ...
        title={currentLanguage === 'Eng' ? 'Group1' : 'Group1'}
      />
      <GridComponent
        dataSource={data}
        width="auto"
      ...
      <div className="mt-4 flex justify-end">
        <button type="button" onClick={() => navigate('/cnpractice/create')} className="bg-blue-500 text-white px-6 py-2 rounded-md">{currentLanguage === 'Eng' ? 'Create Post' : '글쓰기'}</button>
      </div>
    </div>
  );
};

export default Cnpractice;
